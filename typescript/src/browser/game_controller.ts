...
export class GameController {
  constructor(private page: Page, private gameType: GameType) {}

  async clickCell(index: number, board: BoardState): Promise<void> {
  ...
  async flagCell(index: number, board: BoardState): Promise<void> {
    const width = board.width || 9;
    const col = (index % width) + 1;
    const row = Math.floor(index / width) + 1;
    const id = `${row}_${col}`;
  ...
  async chordCell(index: number, board: BoardState): Promise<void> {
  ...
  private async cellCoords(index: number, board: BoardState): Promise<{ x: number; y: number }> {
